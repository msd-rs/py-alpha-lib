...
#[test]
fn test_runtime_split_factor() -> Result<()> {
  let code = r#"
    BW : BW_SPLIT_FACTOR(PRICE, DIV, TS, RS, RP);
    FW : FW_SPLIT_FACTOR(PRICE, DIV, TS, RS, RP);
  "#;
  ...
  assert_eq!(lines.len(), 2);
  assert_eq!(lines[0].name, "bw");
  assert_eq!(lines[0].data, vec![1.0, 2.0, 20.0 / 9.0]);
  assert_eq!(lines[1].name, "fw");
  assert_eq!(lines[1].data, vec![0.45, 0.9, 1.0]);
