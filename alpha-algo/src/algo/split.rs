  ...
  r.par_chunks_mut(group_size)
    ...
    .for_each(|(((((r, p), div), ts), rs), rp)| {
      ...
      for t in (start + 1..end).rev() {
        let d = if is_normal(&div[t]) { div[t] } else { NumT::zero() };
        let ts_val = if is_normal(&ts[t]) { ts[t] } else { NumT::zero() };
        let rs_val = if is_normal(&rs[t]) { rs[t] } else { NumT::zero() };
        let rp_val = if is_normal(&rp[t]) { rp[t] } else { NumT::zero() };
