...
fn build_py_bindings(functions: &[TaFunc]) -> Result<()> {
  ...
  for func in functions {
    ...
    if py_func_name == "ref" || py_func_name == "where" {
    ...
    if arrays.len() == 6 {
      ...
      writeln!(
        code,
        "    if let Some((((((mut {}, {}), {}), {}), {}), {})) = {}",
        r_name, a_name, b_name, c_name, d_name, e_name, r_name
      )?;
      ...
      for name in &[a_name, b_name, c_name, d_name, e_name] {
        writeln!(code, "      let {} = {}.as_array();", name, name)?;
        writeln!(
          code,
      ...
      writeln!(
        ...
        rust_func_name, r_name, a_name, b_name, c_name, d_name, e_name, args_f64
      )?;
      ...
      writeln!(
        code,
        "      {}(&ctx, {}, {}, {}, {}, {}, {}{}).map_err(|e| e.into())",
        rust_func_name, r_name, a_name, b_name, c_name, d_name, e_name, args_f32
      )?;
    ...
    if arrays.len() == 4 {
      ...
      let c_is_bool = matches!(arrays[3].1, TaType::BoolArray(_));
      ...
      if !r_is_bool && a_is_bool && !b_is_bool && !c_is_bool {
        // WHERE case: r=num, a=bool, b=num, c=num
        // f64
        writeln!(
          code,
          "    if let Some((((mut {}, {}), {}), {})) = {}",
        ...
        // List support for (r: NumArray, a: BoolArray, b: NumArray, c: NumArray)
        writeln!(
          code,
          "    }} else if let Some(((({}, {}), {}), {})) = {}.cast::<PyList>().ok().zip({}.cast::<PyList>().ok()).zip({}.cast::<PyList>().ok()).zip({}.cast::<PyList>().ok()) {{",
          r_name, a_name, b_name, c_name, r_name, a_name, b_name, c_name
        )?;
        writeln!(
          code,
          "      if {}.len() != {}.len() || {}.len() != {}.len() || {}.len() != {}.len() {{ return Err(PyValueError::new_err(\"length mismatch\")); }}",
          r_name, a_name, r_name, b_name, r_name, c_name
        )?;
        ...
        writeln!(
          code,
          "        let {} = {}.extract::<Vec<PyReadonlyArray1<'py, bool>>>()?;",
          a_name, a_name
        )?;
        writeln!(code, "        ctx._groups = {}.len() as u32;", r_name)?;
        writeln!(code, "        for i in 0..{}.len() {{", r_name)?;
        writeln!(
          code,
          "          let mut r_arr = {}[i].as_array_mut();",
          r_name
        )?;
        writeln!(
          code,
        ...
        writeln!(
          code,
          "          {}(&ctx, r_slice, a_slice, b_slice, c_slice{}).map_err(|e| PyErr::from(e))?;",
          rust_func_name, args_f64
        )?;
        writeln!(code, "        }}")?;
        writeln!(code, "        Ok(())")?;
    ...
    let gen_args = |type_name: &str| -> String {
      ...
      for param in &func.params {
        match param {
          TaType::NumArray(_) | TaType::BoolArray(_) | TaType::Context(_) => {} // Handled separately
          TaType::Num(n) => {
            if type_name == "f32" {
              let _ = write!(args, ", {} as f32", n);
            } else {
    ...
    if r_is_bool && input_is_bool {
      // Single array: bool output with bool input (e.g. ALL, ANY, BACKSET, FILTER)
      writeln!(
        code,
        "    if let Some((mut {}, {})) = {}",
        r_name, input_name, r_name
      )?;
      writeln!(code, "      .extract::<PyReadwriteArray1<'py, bool>>()")?;
      writeln!(code, "      .ok()")?;
      writeln!(
        code,
        "      .zip({}.extract::<PyReadonlyArray1<'py, bool>>().ok())",
      ...
      // PyList support for bool output with bool input
      writeln!(
        code,
  ...
  for func in functions {
    if func.name == "ref" || func.name == "where" {
      writeln!(
        code,
...
fn build_algo_py(functions: &[TaFunc]) -> Result<()> {
  ...
  for func in functions {
    ...
    if arrays.len() == 6 {
      ...
      writeln!(
        file,
        "  if isinstance({}, list) and isinstance({}, list) and isinstance({}, list) and isinstance({}, list) and isinstance({}, list):",
    ...
    if arrays.len() == 4 {
      ...
      let dtype = if r_is_bool { ", dtype=bool" } else { "" };
    ...
    if r_is_bool && input_is_bool {
      writeln!(
        file,
        "    {} = [np.empty_like(x, dtype=bool) for x in {}]",
        r_name, input_name
      )?;
      writeln!(
        file,
...
fn build_algo_lua(functions: &Vec<TaFunc>) -> Result<()> {
  ...
  for func in functions {
    ...
    let has_num_param = func
      .params
      .iter()
      .any(|p| matches!(p, TaType::NumArray(_) | TaType::Num(_)));
    let call_expr = if has_num_param {
      format!("{}::<f64>({})", rust_name, call_args_str)
    } else {
      format!("{}({})", rust_name, call_args_str)
    };
