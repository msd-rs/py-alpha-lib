...
def test_bw_split_factor():
    ...
    al.set_ctx(flags=0, groups=1)
    res = al.BW_SPLIT_FACTOR(price, dividend, transfer_shares, right_shares, right_price)
    
    assert np.allclose(res, np.array([1.0, 2.0, 20.0 / 9.0]))

def test_fw_split_factor():
    price = np.array([10.0, 10.0, 10.0], dtype=np.float64)
    dividend = np.array([0.0, 0.0, 1.0], dtype=np.float64)
    transfer_shares = np.array([0.0, 1.0, 0.0], dtype=np.float64)
    right_shares = np.array([0.0, 0.0, 0.0], dtype=np.float64)
    ...
    al.set_ctx(flags=0, groups=1)
    res = al.FW_SPLIT_FACTOR(price, dividend, transfer_shares, right_shares, right_price)
    
    assert np.allclose(res, np.array([0.45, 0.9, 1.0]))
