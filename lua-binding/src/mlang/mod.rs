...
fn contains_variable(pair: &Pair<Rule>) -> bool {
    match pair.as_rule() {
        ...
        _ => pair.clone().into_inner().any(|child| contains_variable(&child)),
    }
}
...
fn translate_expr(pair: Pair<Rule>, vars: &HashSet<String>) -> String {
    match pair.as_rule() {
        ...
        Rule::atom => {
            translate_expr(pair.into_inner().next().unwrap(), vars)
        }
        Rule::neg => {
        ...
        Rule::dotted_name => {
            translate_dotted_name(pair, vars)
        }
        Rule::func_call => {
            translate_func_call(pair, vars)
        }
        Rule::identifier => {
...
pub fn to_lua(code: &str) -> Result<String> {
    if code.trim().is_empty() {
        return Ok(r#"function compute(D, P)
  local lines = {}
  return lines
end"#
        .to_string());
